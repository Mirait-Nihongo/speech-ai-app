...
# --- 設定 ---
st.set_page_config(page_title="日本語発音 指導補助ツール v2.3", page_icon="👨‍🏫", layout="centered")
...
def ask_gemini(student_name, nationality, text, alts, details):
    try:
        ...
        【出力形式（厳守）】
        レポートの冒頭に、必ず以下のフォーマットでサマリーを出力し、その後に詳細分析を続けてください。

        ### 【総合評価サマリー】
        ・総合発音スコア： [ここに0~100の数値を算出] / 100
        ・明瞭度： [S/A/B/C]（[短い評価コメント]）
        ・日本語らしさ（リズム・拍）： [S/A/B/C]
        ・要重点指導音： [特に改善すべき音や項目を列挙]
